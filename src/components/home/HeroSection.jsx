      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 lg:gap-12 items-center">
          ...
          <div className="lg:col-span-6 space-y-3 sm:space-y-5 text-right order-1 lg:order-1">
            ...
            {/* عناصر الضمان الأربعة مدمجة في سطر واحد على الموبايل والديسكتوب */}
            <div className="grid grid-cols-4 gap-1 sm:gap-2 pt-2.5 sm:pt-4 border-t border-[#EADFD5]">
              <div className="flex flex-col sm:flex-row items-center justify-center sm:justify-start gap-1 p-1.5 sm:p-2.5 bg-white/95 rounded-xl border border-[#EADFD5] shadow-2xs text-center sm:text-right">
                <Check className="text-[#2D6A4F] shrink-0" size={13} />
                <span className="text-[10px] sm:text-xs font-bold text-[#3D403D] leading-tight">أصلية 100%</span>
              </div>
              <div className="flex flex-col sm:flex-row items-center justify-center sm:justify-start gap-1 p-1.5 sm:p-2.5 bg-white/95 rounded-xl border border-[#EADFD5] shadow-2xs text-center sm:text-right">
                <ShieldCheck className="text-[#2D6A4F] shrink-0" size={13} />
                <span className="text-[10px] sm:text-xs font-bold text-[#3D403D] leading-tight">معاينة الشحنة</span>
              </div>
              <div className="flex flex-col sm:flex-row items-center justify-center sm:justify-start gap-1 p-1.5 sm:p-2.5 bg-white/95 rounded-xl border border-[#EADFD5] shadow-2xs text-center sm:text-right">
                <Headphones className="text-[#2D6A4F] shrink-0" size={13} />
                <span className="text-[10px] sm:text-xs font-bold text-[#3D403D] leading-tight">استشارة مجانية</span>
              </div>
              <div className="flex flex-col sm:flex-row items-center justify-center sm:justify-start gap-1 p-1.5 sm:p-2.5 bg-white/95 rounded-xl border border-[#EADFD5] shadow-2xs text-center sm:text-right">
                <Heart className="text-[#A67C5B] shrink-0" size={13} />
                <span className="text-[10px] sm:text-xs font-bold text-[#3D403D] leading-tight">لأنكِ تستحقين</span>
              </div>
            </div>
          </div>

          {/* الجانب الأيسر: كادر الفيديو السينمائي بنسبة عرض كاملة وواضحة */}
          <div className="lg:col-span-6 order-2 lg:order-2 flex justify-center pt-2 sm:pt-0">
            <div className="relative w-full max-w-sm lg:max-w-lg aspect-video sm:aspect-square rounded-2xl sm:rounded-3xl overflow-hidden shadow-xl shadow-[#A67C5B]/15 border-2 sm:border-4 border-white bg-black">
              <video
